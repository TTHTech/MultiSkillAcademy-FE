  ...
  useEffect(() => {
    const fetchTopicCourses = async () => {
      try {
        ...
        setError("");
        console.log(
          `Fetching courses from: ${baseUrl}/api/student/courses/newest`
        );

        const response = await axios.get(
          `${baseUrl}/api/student/courses/newest`
        );
        ...
        if (Array.isArray(response.data)) {
          ...
          const possibleCourses = Object.values(response.data).find(
            (v) => Array.isArray(v) && v.length > 0 && v[0].courseId
          );
  ...
  return (
    <div className="w-full h-full min-h-screen bg-white overflow-y-auto mt-[90px]">
      ...
      <div className="container mx-auto p-6">
        <h1 className="text-3xl font-bold mb-6">Danh sách các khóa học mới</h1>
...
export default NewestCoursesPage;
