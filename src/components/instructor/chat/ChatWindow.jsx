...
const ChatWindow = ({
  ...
  const fetchMessages = async (chatId) => {
    try {
      ...
      const response = await fetch(`${baseUrl}/api/instructor/chat/${chatId}`, {
        headers: {
  ...
  const getFullFileUrl = (fileUrl) => {
    ...
    if (fileUrl && fileUrl.includes("/api/instructor/chat/files/")) {
      ...
      return `${baseUrl}/api/instructor/chat/files/${fileName}`;
    }
    ...
    if (fileUrl && fileUrl.includes("image_")) {
      const imageId = fileUrl.includes("/")
        ? fileUrl.split("/").pop()
        : fileUrl;
      return `${baseUrl}/api/instructor/chat/files/${imageId}`;
    }
  ...
  const handleDeleteMessage = async (messageId) => {
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/instructor/chat/${chatData.chatId}/message/${messageId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const addMessage = async (content, file, messageType = "TEXT") => {
    try {
      ...
      if (messageType === "TEXT") {
        response = await fetch(`${baseUrl}/api/instructor/chat/message/text`, {
          method: "POST",
          headers: {
        ...
        response = await fetch(
          `${baseUrl}/api/instructor/chat/${chatData.chatId}/message/media`,
          {
            method: "POST",
