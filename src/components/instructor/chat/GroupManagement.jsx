...
const GroupManagement = ({ chatData, onClose, onUpdate }) => {
  ...
  const getAvailableUsers = () => {
    ...
    return allUsers.filter((user) => {
      ...
      // Instructor can only add Students or Admins (not other Instructors)
  ...
  const deleteGroup = async () => {
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/instructor/chat/group/${chatData.chatId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          ...
                          {participant.participantType === 'ADMIN' && " - Người tạo nhóm"}
                        </p>
                      </div>
                    </div>
