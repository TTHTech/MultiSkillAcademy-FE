...
const ChatMessage = ({
  ...
  return (
    <div className={`group flex ${isInstructor ? "justify-end" : "justify-start"} mb-4`}>
      <div className={`flex ${isInstructor ? "flex-row-reverse" : "flex-row"} items-end max-w-[70%] relative`}>
        {!isInstructor && (
          <img
            src={userAvatar}
            alt="Avatar"
        ...
        <div className="relative">
          {/* Hiển thị tên người gửi phía trên tin nhắn cho những người không phải instructor trong group chat */}
          {isGroup && !isInstructor && senderName && (
          ...
          {/* Message Content */}
          {messageType === "IMAGE" ? (
            <div className={`rounded-2xl overflow-hidden ${isInstructor ? "ml-2" : ""} bg-gray-100`}>
              {!imgLoaded && !imgError && (
            ...
            <div className={`rounded-2xl overflow-hidden ${isInstructor ? "ml-2" : ""}`}>
              <video
                src={fileUrl}
                controls
            ...
            <div className={`p-3 rounded-2xl ${isInstructor ? "bg-purple-500 text-white ml-2" : "bg-gray-100 text-gray-900"}`}>
              <a
                href={fileUrl}
                target="_blank"
              ...
              >
                ...
                <span className="flex flex-col">
                  ...
                  <span className={`text-xs mt-1 break-all ${isInstructor ? "text-purple-100" : "text-gray-500"}`}>
                    {fileUrl.split('/').pop()}
                  </span>
                </span>
              </a>
          ...
          {/* Hover Menu Button - Only for instructor's own messages */}
          {isInstructor && (
            <div className={`absolute top-0 ${isInstructor ? "-left-10" : "-right-10"} opacity-0 group-hover:opacity-100 transition-opacity message-menu-container`}>
              <button
              ...
              {/* Inline Menu */}
              {showMenu && (
                <div className={`absolute ${isInstructor ? "right-0" : "left-0"} top-8 bg-white shadow-lg rounded-lg py-2 z-50 border min-w-[160px]`}>
                  <button
          ...
          {/* Timestamp */}
          <div className={`mt-1 ${isInstructor ? "text-right" : "text-left"}`}>
