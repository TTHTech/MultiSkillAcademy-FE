...
const ResourceDescription = () => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        ...
        setCourse(data);
        setTargetAudience(data.resourceDescription || []);
      } catch (err) {
  ...
  const handleSave = async (updatedAudience) => {
    ...
    const instructorCoursesDetailDTO = {
      ...
      resourceDescription: audienceToSave,
  ...
  return (
    <div className="min-h-screen bg-white-100 p-6">
      <h1 className="text-2xl font-bold mb-4">Mô tả tài nguyên khóa học</h1>
      ...
      <p className="font-bold mt-8 mb-2">
        Mô tả tài nguyên có trong khóa học của bạn.
      </p>
      ...
      <div className="mt-6">
        <h2 className="text-lg font-semibold">
          Thêm mô tả tài nguyên mà khóa học bạn có
        </h2>
        <p className="text-gray-500 text-sm italic mb-2">
          Hãy đảm bảo thông tin bạn thêm sẽ đúng với nối dung của khóa học của
          bạn.
        </p>
        <div className="flex items-center space-x-2 p-2 border border-gray-300 rounded bg-white">
          <input
            ...
            className="flex-1 outline-none border-none text-gray-700"
            placeholder="Nhập thông tin tài nguyên (tối đa 160 ký tự)"
          />
          <span className="border border-purple-500 text-purple-500 rounded px-2 py-1 text-sm">
            {newAudience.length}
          </span>
