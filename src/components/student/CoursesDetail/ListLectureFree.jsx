...
const ListLectureFree = ({ onClose }) => {
    ...
    const fetchLectures = async () => {
      try {
        ...
        if (Array.isArray(response.data) && response.data.length > 0) {
          ...
          setSelectedLectureId(response.data[0].id);
        } else {
  ...
  if (error) return <ErrorScreen message={error} onClose={onClose} />;

  return (
    <div
      className="fixed inset-0 bg-black/90 backdrop-blur-sm flex justify-center items-center z-50 p-4 mt-[90px]"
      onClick={onClose}
    >
      <div
        className="bg-white rounded-2xl w-full max-w-6xl h-[85vh] overflow-hidden shadow-2xl relative"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        ...
        <div className="h-full pt-16 flex">
          ...
          <div className="flex-1 bg-gray-900 flex items-center justify-center p-6">
            <VideoPlayer
              url={lectures.find((l) => l.id === selectedLectureId)?.videoUrl}
            />
          </div>
        </div>
      </div>
