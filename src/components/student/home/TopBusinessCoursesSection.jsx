...
const TopBusinessCoursesSection = () => {
  ...
  const categoryId = "CAT002"; // Business category ID
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        setError("Không thể tải dữ liệu khóa học kinh doanh.");
        console.error("Failed to fetch business courses", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <section className="py-12 px-6 bg-gradient-to-r from-amber-50 to-teal-50">
        <div className="max-w-[1500px] mx-auto flex justify-center items-center min-h-[400px]">
          <div className="flex flex-col items-center gap-4">
            <Loader2 className="w-8 h-8 text-amber-600 animate-spin" />
            <p className="text-gray-600">Đang tải khóa học kinh doanh...</p>
          </div>
        </div>
      </section>
    );
      ...
      <div className="max-w-[1500px] mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-start gap-4">
            <Briefcase className="w-12 h-12 text-amber-600 mt-1" />
            <div>
              <h2 className="text-4xl font-extrabold bg-gradient-to-r from-amber-600 to-teal-600 bg-clip-text text-transparent">
                Kinh Doanh & Khởi Nghiệp
              </h2>
              <p className="text-gray-700 mt-2 font-medium">
                Phát triển kỹ năng kinh doanh cùng các chuyên gia hàng đầu và doanh nhân thành công
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`p-3 rounded-md transition-all duration-300 ${
                isLeftVisible
                  ? "bg-white shadow-lg hover:shadow-xl text-gray-800 hover:bg-amber-50"
...
export default TopBusinessCoursesSection;