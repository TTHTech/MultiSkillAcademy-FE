...
const TopMarketingCoursesSection = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        setError("Không thể tải dữ liệu khóa học marketing.");
        console.error("Failed to fetch marketing courses", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <section className="py-12 px-6 bg-gradient-to-r from-emerald-50 to-teal-50">
        <div className="max-w-[1500px] mx-auto flex justify-center items-center min-h-[400px]">
          <div className="flex flex-col items-center gap-4">
            <Loader2 className="w-8 h-8 text-emerald-600 animate-spin" />
            <p className="text-gray-600">Đang tải khóa học marketing...</p>
          </div>
        </div>
      </section>
    );
      ...
      <div className="max-w-[1500px] mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-start gap-4">
            <TrendingUp className="w-12 h-12 text-emerald-500 mt-1" />
            <div>
