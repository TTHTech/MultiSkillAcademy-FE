...
import { Star, ChevronLeft, ChevronRight, Trophy, Flame, Code2, Loader2 } from 'lucide-react';
...
const TopTechnologyCoursesSection = () => {
  ...
  const categoryId = "CAT004"; // Technology category ID
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        setError("Không thể tải dữ liệu khóa học công nghệ.");
        console.error("Failed to fetch technology courses", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <section className="py-12 px-6 bg-gradient-to-r from-blue-50 to-cyan-50">
        <div className="max-w-[1500px] mx-auto flex justify-center items-center min-h-[400px]">
          <div className="flex flex-col items-center gap-4">
            <Loader2 className="w-8 h-8 text-blue-600 animate-spin" />
            <p className="text-gray-600">Đang tải khóa học công nghệ...</p>
          </div>
        </div>
      </section>
    );
      ...
      <div className="max-w-[1500px] mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-start gap-4">
            <Code2 className="w-12 h-12 text-blue-600 mt-1" />
            <div>
              <h2 className="text-4xl font-extrabold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">
                Công Nghệ & Lập Trình
              </h2>
              <p className="text-gray-700 mt-2 font-medium">
                Khám phá các khóa học công nghệ từ các chuyên gia hàng đầu trong ngành
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`p-3 rounded-md transition-all duration-300 ${
                isLeftVisible
                  ? 'bg-white shadow-lg hover:shadow-xl text-gray-800 hover:bg-blue-50'
...
export default TopTechnologyCoursesSection;