...
import { Bell, Calendar, Check, Clock, AlertTriangle, ChevronLeft, ChevronRight, Mail, MessageSquare, Save } from "lucide-react";
const baseUrl = import.meta.env.VITE_REACT_APP_BASE_URL;

const UpdateStudyReminder = ({ Data, closeModal, triggerRefresh, courseId }) => {
  const [step, setStep] = useState(1);
  const [formData, setFormData] = useState({
    content: Data.content || "",
    email: Data.email || "",
    courseId: Data.courseId || courseId,
    frequency: Data.frequency || "Hàng ngày",
    time: Data.time || [9, 0],
    selectedDays: Data.selectedDays || [],
    isActive: Data.isActive !== undefined ? Data.isActive : true,
  });
  const [showCustomInput, setShowCustomInput] = useState(false);
  const [isContentSelected, setIsContentSelected] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);
  
  useEffect(() => {
    // Kiểm tra xem content có phải là một trong những lựa chọn có sẵn không
    ...
    if (!predefinedContents.includes(formData.content)) {
  ...
  const handleFrequencyClick = (frequency) => {
    setFormData((prev) => ({ 
      ...prev, 
      frequency,
      ...(frequency !== "Hàng tuần" ? { selectedDays: [] } : {})
    }));
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.put(
        `${baseUrl}/api/student/reminders/${Data.id}`,
        payload
      );
          ...
          <div className="space-y-6">
            <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100 mb-4">
              <div className="flex items-start">
                ...
                <div>
                  <h3 className="font-medium text-indigo-800 mb-1">Nội dung thông báo hiện tại</h3>
                  <p className="text-gray-800 font-medium">{formData.content || "Chưa có nội dung thông báo."}</p>
                </div>
              </div>
            </div>
            
            <div>
              <p className="text-sm font-medium text-gray-700 mb-3">Chọn nội dung thông báo mới</p>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center">
                <div className="relative flex-1">
                  <select
                    name="timeHour"
                    value={String(formData.time[0]).padStart(2, "0")}
                    onChange={(e) => setFormData({
                      ...formData,
                      time: [parseInt(e.target.value), formData.time[1]],
                    })}
                ...
                <div className="relative flex-1">
                  <select
                    name="timeMinutes"
                    value={String(formData.time[1]).padStart(2, "0")}
                    onChange={(e) => setFormData({
                      ...formData,
                      time: [formData.time[0], parseInt(e.target.value)],
                    })}
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-700 font-medium">{`${String(formData.time[0]).padStart(2, "0")}:${String(formData.time[1]).padStart(2, "0")}`}</span>
                  </div>
                </div>
              </div>
            </div>
...
export default UpdateStudyReminder;