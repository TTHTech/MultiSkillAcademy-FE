  ...
  const handleDelete = async (userId) => {
    ...
    if (confirmed) {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          `http://localhost:8080/api/admin/students/${userId}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {user.profileImage ? (
                      ...
                      <div
                        className={`w-10 h-10 flex items-center justify-center rounded-full ${getRandomColor()} text-white text-lg font-semibold`}
                      >
                        {user.lastName
